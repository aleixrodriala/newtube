...
echo "Starting $duration-second device soak: $video_url"
if [[ -n "$forced_client" ]]; then
  echo "Forcing /player client: $forced_client"
  adb shell setprop debug.arc.player_client "$forced_client"
  ...
  adb shell setprop debug.arc.player_client none
fi
adb shell setprop debug.arc.fresh_app_info "$fresh_app_info"
...
adb shell setprop debug.arc.poison_itag none
if [[ -n "$poison_once_itag" ]]; then
  echo "Injecting one synthetic 403 for itag: $poison_once_itag"
  adb shell setprop debug.arc.poison_once_itag "$poison_once_itag"
else
  adb shell setprop debug.arc.poison_once_itag none
fi
cleanup() {
  adb shell setprop debug.arc.player_client none >/dev/null
  adb shell setprop debug.arc.fresh_app_info 0 >/dev/null
  adb shell setprop debug.arc.poison_itag none >/dev/null
  adb shell setprop debug.arc.poison_once_itag none >/dev/null
}
trap cleanup EXIT
adb logcat -c
...
adb shell am start -W -a android.intent.action.VIEW -d "$video_url" -p "$package" >/dev/null

deadline=$((SECONDS + duration))
while (( SECONDS < deadline )); do
  if adb logcat -d -v brief -s NetPath:D | grep -q 'auto-reload cap hit'; then
    break
  fi
  sleep 5
done

adb logcat -d -v time -s NetPath:D VideoInfoService:D | grep -E \
